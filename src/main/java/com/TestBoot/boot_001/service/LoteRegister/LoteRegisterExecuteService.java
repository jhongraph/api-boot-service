...
@Slf4j
@Service("register")
public class LoteRegisterExecuteService implements ServiceExecute {

    @Autowired
    private final LoteRegisterService register;
    @Autowired
    ...
    public void execute(PreSaleRequest request) throws InterruptedException {
        ...
        Thread.currentThread().setName("REGISTER-BOT");
        ...
        for (int i = 0; i < request.getConcurrency(); i++) {
            ...
            try {
                ...
                register.executeTask(request, latch);
