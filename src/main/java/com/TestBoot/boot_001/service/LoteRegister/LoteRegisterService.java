...
@Slf4j
@Service
public class LoteRegisterService extends BaseExecut {

    private final Env env;
    private final AutoFillRegisterService autoFill;
    private final UploadFileService uploadFiles;
    private final DigitalCheckService digitalCheck;
    private final DomElement element;
    private final WebSocketSender sender;

    public LoteRegisterService(Env env, DomElement element, WebSocketSender sender, AutoFillRegisterService autoFill, DigitalCheckService digitalCheck, UploadFileService uploadFiles) {
        super(env, element);
        this.env = env;
    ...
    public void executeTask(PreSaleRequest request, CountDownLatch latch) {
        ...
        try {
            ...
            while (createdCasesByUser < request.getProcessQuantity()) {
                try {
                    if (success) {
                        login(request.getUsername(), request.getPassword(), request.getUrlBase(), element.getRegisterButton(), driver, wait);
                        success = false;
                    }
                    autoFill.fillFormAndContinue(driver, wait);

                    digitalCheck.performSignatureStep(wait, element.getRegistrationDigitalCheckButton1(), null, null);
                    ...
                    if (realCause instanceof LoginException ||
                        ...
                        sender.sendLog("error", service, message);
                        throw new LoteRegisterException("ERROR: ", e);
                    }
                    success = true;
                }
            }
